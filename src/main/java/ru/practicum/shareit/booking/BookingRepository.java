...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("select b from Booking b join Item i on b.itemId = i.id where i.owner = :id order by b.start desc")
    List<Booking> findBookingsByOwner(@Param("id") Long ownerId);

    @Query("select b from Booking b join Item i on b.itemId = i.id where i.owner = :id " +
